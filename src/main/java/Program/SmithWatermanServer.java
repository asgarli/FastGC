    ...
    protected void interpretResult() throws Exception {
        ...
        for (int i = 0; i < outLabels.length; i++) {
            if (outputState.wires[i].value != Wire.UNKNOWN_SIG) {
                ...
                output = output.setBit(i);
            } else if (!outLabels[i].equals(outputState.wires[i].invd ? outputState.wires[i].lbl.xor(Wire.R
                    .shiftLeft(1).setBit(0)) : outputState.wires[i].lbl))
                throw new Exception("Bad label encountered: i = " + i + "\t" + outLabels[i] + " != ("
                        + outputState.wires[i].lbl + ", " + outputState.wires[i].lbl.xor(Wire.R.shiftLeft(1).setBit(0))
                        + ")");
        }
    ...
    protected void verify_result() throws Exception {
        ...
        for (int i = 1; i < sdnaLen + 1; i++)
            for (int j = 1; j < cdnaLen + 1; j++) {
                int s = SmithWatermanCommon.score[SmithWatermanCommon.codons.indexOf(sdnaStr.charAt(i - 1))][SmithWatermanCommon.codons
                        .indexOf(cdnaStr.charAt(j - 1))];
                D[i][j] = 0;
                for (int o = 1; o <= i; o++) {
