...
public class EditDistanceServer extends ProgServer {
    ...
    public EditDistanceServer(BigInteger dna, int length) {
        ...
        // length of dna sequence. Effective bit length of variable dna is twice
        // of 'length'
    ...
    protected void interpretResult() throws Exception {
        ...
        for (int i = 0; i < outLabels.length; i++) {
            if (outputState.values[i] != Wire.UNKNOWN_SIG) {
                ...
                continue;
            } else if (outLabels[i].equals(outputState.invd[i] ? outputState.lbls[i] : outputState.lbls[i].xor(Wire.R
                    .shiftLeft(1).setBit(0)))) {
                output = output.setBit(i);
            } else if (!outLabels[i].equals(outputState.invd[i] ? outputState.lbls[i]
                    .xor(Wire.R.shiftLeft(1).setBit(0)) : outputState.lbls[i]))
                throw new Exception("Bad label encountered: i = " + i + "\t" + outLabels[i] + " != ("
                        + outputState.lbls[i] + ", " + outputState.lbls[i].xor(Wire.R.shiftLeft(1).setBit(0)) + ")");
        }
    ...
    protected void verify_result() throws Exception {
        ...
        for (int i = 1; i < EditDistanceCommon.sdnaLen + 1; i++)
            for (int j = 1; j < EditDistanceCommon.cdnaLen + 1; j++) {
                ...
                D[i][j] = Math.min(Math.min(D[i - 1][j] + 1, D[i][j - 1] + 1), D[i - 1][j - 1] + t);
            }

        System.out.println("output (verify): " + D[EditDistanceCommon.sdnaLen][EditDistanceCommon.cdnaLen]);
    }
}