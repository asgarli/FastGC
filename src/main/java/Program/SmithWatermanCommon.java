...
public class SmithWatermanCommon extends ProgCommon {
    ...
    static int maxscore;
    public static int nBits; // the number of bits used to denote an entry in
                             // the score matix.
    static int gapA = 12, gapB = 7; // gap function g(k) = A+B*k, A >= 0, B >=
                                    // 0.
    ...
    static BigInteger toBigInteger(String dna) {
        ...
        for (int i = dna.length() - 1; i >= 0; i--) {
            res = res.shiftLeft(nBitsCodon).xor(BigInteger.valueOf(codons.indexOf(dna.charAt(i))));
        }
    ...
    private static State computeSimilarityWithGap(State[][] H, int row, int col, int gap) {
        ...
        if (stG.largerThan(stH)) {
            return new State(BigInteger.ZERO, widthOfEntry(row, col));
        } else if (stH.plainValue != null && stG.plainValue != null && stH.plainValue.compareTo(stG.plainValue) > 0) {
    ...
    private static State computeSimilarityWithScoreMatrix(State[][] H, int row, int col, BigInteger[] sdnalbs,
            BigInteger[] cdnalbs) {
        ...
        for (int l = 2; l <= sdnaLen + cdnaLen; l++) {
            for (int m = 1; m <= l; m++) {
                ...
                if (i >= 1 && i <= sdnaLen && j >= 1 && j <= cdnaLen) {
                    ...
                    State stScore = computeSimilarityWithScoreMatrix(H, i, j, sdnalbs, cdnalbs);
